...
function getEmptyBook(title, imgName, amount, isOnSale) {
    return {
        id: '',
        title,
        description: 'lorem euismod dictumst inceptos mi',
        thumbnail: `http://coding-academy.org/books-photos/${imgName}.jpg`,
        listPrice: {
            amount,
            currencyCode: 'EUR',
            isOnSale
...
function _createBooks() {
    ...
    if (!books || !books.length) {
        ...
        // books.push(_createBook('metus hendrerit', 20, 109, false))
        // books.push(_createBook('morbi', 14, 44, true))
        // books.push(_createBook('dictum', 16, 30, true))
        // books.push(_createBook('sem himenaeos aptent', 12, 19, false))

        utilService.saveToStorage(BOOK_KEY, books)
    }
}

function _createBook(title, imgName, amount, isOnSale) {
    const book = getEmptyBook(title, imgName, amount, isOnSale)
